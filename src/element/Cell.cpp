...
Cell::Cell(const ContainerPrtPoints& cPoints, elemType type, unsigned elemID) : Element(cPoints, type, elemID) {
...
void Cell::calcVolumeCentroid(ContainerFaces& faces) {
    ...
    double sign;
    double intergal;
    forAllIter(ContainerFaces, faces, it) {
...
ContainerPrtPoints Cell::findFace(unsigned vertice1, unsigned vertice2, unsigned vertice3, unsigned vertice4)
{
    ContainerPrtPoints result {this->points[vertice1], this->points[vertice2], this->points[vertice3]};
