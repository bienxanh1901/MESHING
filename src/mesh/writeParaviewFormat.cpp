...
void Mesh::writeVTUtest() {
    ...
    ofstream _out;
    _out.open("internalFaces.vtu", std::ofstream::out);
...
//        _out << vtkFunc::HEADER(2) << vtkFunc::PIECE_TAG(this->meshInfo.numberOfPoints, this->meshInfo.numberOfFaces) << "\n";
        _out << vtkFunc::HEADER(2) << vtkFunc::PIECE_TAG(this->meshInfo.numberOfPoints, this->meshInfo.numberOfInternalFaces) << "\n";
...
//
//        }
        for(unsigned i = 0; i < this->meshInfo.numberOfInternalFaces; i++) {

            this->faces[i].printPointsIDParaview(_out);
        }
...
//        for(unsigned i = 1; i<= this->meshInfo.numberOfFaces; i++) {
        for(unsigned i = 1; i<= this->meshInfo.numberOfInternalFaces; i++) {
...
void Mesh::writeVTUtest2() {

    ofstream _out;
    _out.open("boundaryFaces.vtu", std::ofstream::out);
    ...
    if(_out.is_open()) {
        ...
        _out << vtkFunc::HEADER(1) << vtkFunc::TAG(VTU_TYPE) << "\n";
//        _out << vtkFunc::HEADER(2) << vtkFunc::PIECE_TAG(this->meshInfo.numberOfPoints, this->meshInfo.numberOfFaces) << "\n";
        _out << vtkFunc::HEADER(2) << vtkFunc::PIECE_TAG(this->meshInfo.numberOfPoints, this->meshInfo.numberOfFaces - this->meshInfo.numberOfInternalFaces) << "\n";
...
//
//        }
        for(unsigned i = this->meshInfo.numberOfInternalFaces; i < this->meshInfo.numberOfFaces; i++) {
...
//        for(unsigned i = 1; i<= this->meshInfo.numberOfFaces; i++) {
        for(unsigned i = this->meshInfo.numberOfInternalFaces + 1; i<= this->meshInfo.numberOfFaces; i++) {

            _out << (i - this->meshInfo.numberOfInternalFaces)*4 << " ";
